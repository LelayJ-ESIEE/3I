...
public abstract class AbstractControleur {
	...
	protected void afficheAjoutPatient(){	
		mediateur.getControleur(Contr.AJOUTPATIENT).actualiser();
		getMainControleur().getMainBorderPane().setCenter(mediateur.getControleur(Contr.AJOUTPATIENT).getPane());	
	}
	
	protected void afficheSuppressionPatient(){
		mediateur.getControleur(Contr.SUPPRESSIONPATIENT).actualiser();
		getMainControleur().getMainBorderPane().setCenter(mediateur.getControleur(Contr.SUPPRESSIONPATIENT).getPane());			
	}
	
	protected void afficheNote(){
		mediateur.getControleur(Contr.NOTE).actualiser();
		getMainControleur().getMainBorderPane().setCenter(mediateur.getControleur(Contr.NOTE).getPane());			
	}
