...
MOTEUR_INIT	
		ldr R12, = SYSCTL_RCGC0
		ldr	R11, [R12]
       		ORR	R11, R11, #0x00100000  ;;bit 20 = PWM recoit clock: ON (p271) 
        	str R11, [R12]
	...
	;*(int *) (0x400FE060)= *(int *)(0x400FE060)...;
  	...
  	;RCGC2 :  Enable port D GPIO(p291 ) car Moteur Droit sur port D 
		...
		ldr R11, [R12] 		
	        ORR R11, R11, #0x08  ;; Enable port D GPIO 
        	str R11, [R12]
	...
	;MOT2 : RCGC2 :  Enable port H GPIO  (2eme moteurs)
		...
		ldr R12, = SYSCTL_RCGC2
		ldr R11, [R12] 
	        ORR R11, R11, #0x80  	;; Enable port H GPIO 
        	str R11, [R12] 
	...
	;;il faut mettre 1 pour avoir PD0=PWM0 et PD1=PWM1
		...
		ldr R12, = GPIOPCTL_D
		;ldr	R11, [R12] 	 ;;	*(int *)(0x40007000+0x0000052C)=1;
	        ;ORR	R11, R11, #0x01	 ;; Port D, pin 1 = PWM 
		mov R11, #0x01  
        	str R11, [R12]
	...
	;;donc PD0 = 1
		...
		ldr R12, = GPIOAFSEL_D
		ldr R11, [R12] 	  	;*(int *)(0x40007000+0x00000420)= *(int *)(0x40007000+0x00000420) | 0x00000001;
	        ORR R11, R11, #0x01 	
	        str R11, [R12]
	...
	;;Config Modes PWM0 + mode GenA + mode GenB
		...
		ldr R12, = PWM0CTL
		mov R11, #2		;Mode up-down-up-down, pas synchro
	        str R11, [R12]	
		
		ldr R12, =PWM0GENA 	;en decomptage, qd comparateurA = compteur => sortie pwmA=0
	                		;en comptage croissant, qd comparateurA = compteur => sortie pwmA=1
		mov R11,	#0x0B0 		;0B0=10110000 => ACTCMPBD=00 (B down:rien), ACTCMPBU=00(B up rien)
		str R11, [R12]		;ACTCMPAD=10 (A down:pwmA low), ACTCMPAU=11 (A up:pwmA high) , ACTLOAD=00,ACTZERO=00  
		
		ldr R12, =PWM0GENB	;en comptage croissant, qd comparateurB = compteur => sortie pwmA=1
		mov R11,	#0x0B00		;en decomptage, qd comparateurB = compteur => sortie pwmB=0
		str R11, [R12]
	...
	;;on divise par 2 car moteur 6v sur alim 12v
		...
		ldr R12, =PWM0LOAD 	;PWM0LOAD=periode/2 =0x1F4
		mov R11,	#0x1F4
		str R11,[R12]
		
		ldr R12, =PWM0CMPA 	;Valeur rapport cyclique : pour 10% => 1C2h si clock = 0F42400
		mov R11, #VITESSE
		str R11, [R12]  
		
		ldr R12, =PWM0CMPB 	;PWM0CMPB recoit meme valeur. (rapport cyclique depend de CMPA)
		mov R11,	#0x1F4	
		str R11,	[R12]
	...
	;;Config Modes PWM2 + mode GenA + mode GenB
		...
		ldr R12, = PWM1CTL
		mov R11, #2		;Mode up-down-up-down, pas synchro
	        str R11, [R12]		;*(int *)(0x40028000+0x040)=2;
		
		ldr R12, =PWM1GENA 	;en decomptage, qd comparateurA = compteur => sortie pwmA=0
					;en comptage croissant, qd comparateurA = compteur => sortie pwmA=1
		...
		ldr R12, =PWM1GENB	;*(int *)(0x40028000+0x064)=0x0B00;
		mov R11,	#0x0B00		;en decomptage, qd comparateurB = compteur => sortie pwmB=0
		str R11, [R12]		;en comptage croissant, qd comparateurB = compteur => sortie pwmA=1
	...
	;*(int *)(0x40028000+0x050)=0x1F4; //PWM0LOAD=periode/2 =0x1F4
		...
		ldr R12, =PWM1CMPA 	;Valeur rapport cyclique : pour 10% => 1C2h si clock = 0F42400
		mov R11,	#VITESSE
		str	R11, [R12] 	;*(int *)(0x40028000+0x058)=0x01C2;
		
		ldr R12, =PWM1CMPB 	;PWM0CMPB recoit meme valeur. (CMPA depend du rapport cyclique)
		mov R11,	#0x1F4		; *(int *)(0x40028000+0x05C)=0x1F4; 
		str R11,	[R12]
		
	;Control PWM : active PWM Generator 0 (p1167): Enable+up/down + Enable counter debug mod

		ldr R12, =PWM1CTL 
		ldr R11, [R12]		;*(int *) (0x40028000+0x40)= *(int *)(0x40028000+0x40) | 0x07;
		ORR R11,	R11,	#0x07
		str R11,	[R12]		
	...
	;PORT D OUTPUT pin0 (pwm)=pin1(direction)=pin2(slow decay)=pin5(12v enable)
		...
		ldr R12, =GPIODIR_D 
		ldr R11, [R12]
		ORR R11,	#(GPIO_0+GPIO_1+GPIO_2+GPIO_5)
		str R11,[R12]
...
MOTEUR_GAUCHE_OFF
		ldr R12,	=PWMENABLE
		ldr R11,	[R12]
		and R11,	#0x0B		;bit 2 à 0
		str R11,	[R12]
		BX LR

MOTEUR_DROIT_ARRIERE
		;Inverse Direction (GPIO_D1)
		ldr R12, =(GPIODATA_D+(GPIO_1<<2)) 
		mov R11, #0
		str R11,[R12]
		BX LR
...
MOTEUR_GAUCHE_ARRIERE
		;Inverse Direction (GPIO_D1)
		ldr R12, =(GPIODATA_H+(GPIO_1<<2)) 
		mov R11, #2 ; contraire du moteur Droit
		str R11,[R12]
		BX LR		
